...
#How many players you want to read and save in DB per time (0 to 2000)

def createDB(myhost, myuser, mypass, Mydb):
    PLAYERS_PER_LOOP = 100
    finish = 0
    #Connecting to a DB in local host to store data.
    db = MySQLdb.connect(host=myhost, user=myuser, passwd=mypass, db=Mydb)
    cursor = db.cursor()

    #creating the csv with the players URL
    ...
    with open('dictNFL.csv','r') as file:
